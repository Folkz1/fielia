...
export default function AfiliadosPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setResult(editingId ? "Afiliado atualizado!" : "Afiliado criado!");
      setShowForm(false);
      fetchData();
    } catch {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Deletar afiliado permanentemente?")) return;
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Gerencie parceiros afiliados e CTAs no blog, chat e newsletter
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {showForm && (
        <div className="bg-gray-900 border border-white/10 rounded-xl p-6 mb-8">
          <h2 className="text-lg font-semibold text-white mb-4">
            {editingId ? "Editar Afiliado" : "Novo Afiliado"}
          </h2>
      ...
      {affiliates.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-white/10 rounded-xl">
          ...
          <p className="text-gray-400">Nenhum afiliado cadastrado</p>
          <p className="text-xs text-gray-600 mt-1">Clique em &quot;Novo Afiliado&quot; para comecar</p>
        </div>
      ) : (
